...
* TODO Future todos
  - [ ] (later) refactor the pipeline for general training; later could add in
    hao's idea
...
* TODO left to finish
  ...
  - [ ] finish the draft except for validation data
  - [ ] check if the best models are on the edge of the search range
  - [ ] update to Jenna (12-8 pm calling it!): only write the unambiguous part:
  ...
  - [ ] run a small n experiment of mortality: remember to use the same
